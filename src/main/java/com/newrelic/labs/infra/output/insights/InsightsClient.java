...
public abstract class InsightsClient {
	...
	private static Logger logger = LoggerFactory.getLogger(InsightsClient.class);
	...
	public boolean post(byte[] payload) {
		...
		try {
			...
			if (status >= 200 && status < 300) {
				...
				System.err.println("Response status is " + response.getStatusLine());
				logger.error("Response status is " + response.getStatusLine());
