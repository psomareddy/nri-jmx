...
public abstract class Integration<T extends Arguments> {
	...
	private static boolean killSignal = false;
	...
	public void run(T argsBean) throws Exception {
		// argsBean contains parsed arguments in case of infra mode or when args are
		// specified on command line params
		// otherwise they contain the default bean properties

		// MODE and VERBOSE must always be specified as ENV variable or CMD line
		// parameters.
		...
		if (mode.equalsIgnoreCase("infra")) {
			...
			File nrConfigFile = new File(configFolder, "newrelic.yml");
		...
		if (mode.equalsIgnoreCase("infra")) {
			...
			if (isListener()) {
				...
				try {
					...
					System.err.println("integration listener cancelled");
				} finally {
					// clean up
				}
			} else {
				...
				try {
					// getting the future's response will block forever unless an exception is
					// thrown
					future.get();
				} catch (InterruptedException e) {
					System.err.println(e.getMessage());
	...
	private void initalizeInsightsOutput(File nrConfigFile) throws Exception {
		System.out.println("Initializing insights ouput");
		try {
